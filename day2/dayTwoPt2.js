const toClean = ["13-16 k: kkkkkgmkbvkkrskhd",
"5-6 p: qpppvzp",
"3-4 p: psppxhlfpvkh",
...
const cleanedOne = toClean.map((str) => {
    return str.split(' ')
})

const bigHolder = []

const cleanedTwo = cleanedOne.forEach((arr) => {
    let littleHolder = []
    const nums = arr[0].split('-').map(num => Number(num))
    const letter = arr[1].slice(0,1)
    littleHolder.push(nums[0], nums[1], letter, arr[2])
    bigHolder.push(littleHolder)  
})

let answer = 0

bigHolder.forEach((arr) => {
    const firstPosition = arr[0] - 1
    const secondPosition = arr[1] - 1
    const letter = arr[2]
    const string = arr[3]
    if ((string.charAt(firstPosition) == letter && string.charAt(secondPosition) !== letter) || (string.charAt(firstPosition) !== letter && string.charAt(secondPosition) == letter)) {
        answer++
    }
})
