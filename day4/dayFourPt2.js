...
// const test = ["eyr:2029 byr:1931 hcl:z cid:128",
...
const test3 = test2.join('').split('split')

const test4 = test3.map((e) => {
    return e.split(' ')
})

const test5 = test4.map((e) => {
    e.pop()
})
...
for (let i = 0; i < test4.length; i++) {
    const holder2 = []
    for (let j = 0; j < test4[i].length; j++) {
        let spl = test4[i][j].split(":")
        holder2.push(spl)
    }
    holder1.push(holder2)
}
...
for (let i = 0; i < holder1.length; i++) {
    const stringHolder = []
    for (let j = 0; j < holder1[i].length; j++) {
        stringHolder.push(holder1[i][j][0])
    }
    const e = stringHolder.join()
    if (e.includes('byr') && e.includes('iyr') && e.includes('eyr') && e.includes ('hgt') && e.includes('hcl') && e.includes('ecl') && e.includes('pid')) {
        cleaned.push(holder1[i])
    } 
}
...
for (let i = 0; i < cleaned.length; i++) {
    ...
    for (let j = 0; j < obj.length; j++) {
        const first = obj[j][0]
        const second = obj[j][1]
        const best = []
        if (
                (first == "byr" && (second >= 1920 && second <= 2002))
                || (first == "iyr" && (second >= 2010 && second <= 2020))
                || (first == "eyr" && (second >= 2020 && second <= 2030))
                || (first == "hgt" && (second.length == 5 && (second.slice(0,3) >= 150 && second.slice(0,3) <= 193)))
                || (first == "hgt" && (second.length == 2 && (second.slice(0,2) >= 59 && second.slice(0,2) <= 76)))
                || (first == "hcl" && second.slice(0,1) == "#" && second.length == 7 && /[0-9a-f]/.test(second.slice(1,7)))
                || (first == "ecl" && (second == "amb" || second == "blu" || second == "brn" || second == "gry" || second == "grn" || second == "hzl" || second == "oth"))
                || (first == "pid" && (isNaN(second) == false && second.length == 9))
                && (first !== "cid")
            ) {
            best.push(first, second)
        }
        if (best.length >= 1) {
            results.push(best)
        }
